...
/*   Updated: 2018/09/03 20:34:55 by mmerabet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			exp_cmd(t_strid *sid, char **res, t_expf *expf)
{
	...
	else if (!pid && !(sid->str[sid->len - 1] = '\0') && !close(fd[0]))
	{
		dup2(fd[1], 1);
		handle_pgid();
		head = ft_lexer(sid->str + 1, ((t_allf *)expf->data)->lexerf);
		if ((efail = ft_astiter(head, &efail, ((t_allf *)expf->data)->iterf)))
			ft_printf_fd(2, "%s: error in command substitution: %s: [%s]\n",
					g_shell->name, ft_strshret(efail), sid->str + 1);
		ft_astdel(&head);
		tcsetpgrp(0, save);
		close(fd[1]);
		exit(efail ? 1 : 0);
